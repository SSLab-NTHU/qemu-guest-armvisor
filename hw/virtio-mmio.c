 ...
 *
 * Copyright (c) 2011-2012 SSLab, NTHU
 * Patch for ARMvisor
 *
 * Author:
...
static uint32_t virtio_mmio_read(void *opaque, target_phys_addr_t offset,
                                 unsigned size)
{
...
static uint32_t virtio_mmio_readb(void *opaque, target_phys_addr_t offset)
{
    return virtio_mmio_read(opaque, offset, 1);
}

static uint32_t virtio_mmio_readw(void *opaque, target_phys_addr_t offset)
{
    return virtio_mmio_read(opaque, offset, 2);
}

static uint32_t virtio_mmio_readl(void *opaque, target_phys_addr_t offset)
{
    return virtio_mmio_read(opaque, offset, 4);
}

static void virtio_mmio_writeb(void *opaque, target_phys_addr_t offset,
                                     uint32_t value)
{
    virtio_mmio_write(opaque, offset, value, 1);
}

static void virtio_mmio_writew(void *opaque, target_phys_addr_t offset,
                                     uint32_t value)
{
    virtio_mmio_write(opaque, offset, value, 2);
}

static void virtio_mmio_writel(void *opaque, target_phys_addr_t offset,
                                     uint32_t value)
{
    virtio_mmio_write(opaque, offset, value, 4);
}

static CPUReadMemoryFunc * const virtio_mmio_readfn[] = { 
    virtio_mmio_readb,
    virtio_mmio_readw,
    virtio_mmio_readl                                                    
};

static CPUWriteMemoryFunc * const virtio_mmio_writefn[] = { 
    virtio_mmio_writeb,
    virtio_mmio_writew,
    virtio_mmio_writel
};
...
static int virtio_init_mmio(VirtIOMMIOProxy *proxy, VirtIODevice *vdev)
{
    ...
    proxy->iomemtype = cpu_register_io_memory(virtio_mmio_readfn,
                                       virtio_mmio_writefn, proxy,
                                       DEVICE_NATIVE_ENDIAN);

    sysbus_init_mmio(&proxy->busdev, 0x1000, proxy->iomemtype);
...
static int virtio_blk_init_mmio(SysBusDevice *dev)
{
    ...
    //fprintf(stderr, "block conf %x, bs %x\n", &proxy->block, (&proxy->block)->bs);
